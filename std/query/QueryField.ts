...
class QueryField {
     ...
     * @param {string} method
     */
    _assignMethod(method: string, ..._param: any) {
        const args = Array.from(arguments).slice(1).map((arg: any) => {
        ...
        if (isMethod) {
            ...
            if (args.length > 0) {
                ...
                if (Array.isArray(thisArg[thisName])) {
                    // clone value
                    value = {};
                    // clone property
                    // e.g. { $multiply: [ "$price", 0.7 ] }
                    Object.defineProperty(value, thisName, { value: thisArg[thisName], configurable: true, enumerable: true, writable: true });
                ...
                value = thisArg[thisName];
                Object.defineProperty(thisArg[method], this._toReference(thisName), { value: value, configurable: true, enumerable: true, writable: true });
            }
            // remove previous property reference
            delete thisArg[thisName];
            // and finally return
            return this;
        }
        // validate if underlying expression a single select expression e.g. { "name": 1 }
        if (thisArg[thisName] === 1) {
            // if method has arguments
            if (args.length > 0) {
                // insert field as first argument e.g.
    ...
    concat(...arg: any) {
        arg.unshift('$concat');
        return this._assignMethod.apply(this, arg);
    }
    /**
     ...
     * @returns this
     */
    subtract(x: ArrayBufferConstructor) {
