...
import {LiteralToken} from "./LiteralToken.ts";
import {Token} from "./Token.ts";
import {IdentifierToken} from "./IdentifierToken.ts";
import {SyntaxToken} from "./SyntaxToken.ts";
...
class ODataParser {
     ...
     */
    parse(str: string, callback: Function) {
        const self = this;
        //ensure callback
        callback = callback || (() => {});
     ...
     * @param {Function} callback
     */
    resolveMember(member: string, callback: Function) {
     ...
     */
    resolveMethod(_method, _args, callback): MethodCallExpression {
        if (typeof callback !== 'function')
            //sync process
            return null;
        else
     ...
     */
    // eslint-disable-next-line no-unused-vars
    parseBinaryString(value): LiteralToken {
     ...
     * @returns {LiteralToken}
     */
    parseDateTimeOffsetString(value): LiteralToken {
     ...
     * @returns {LiteralToken}
     */
    parseDateTimeString(value): LiteralToken {
        if (value == null)
            return null;
