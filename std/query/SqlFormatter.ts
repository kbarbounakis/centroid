...
import { SqlUtils } from './SqlUtils.ts';
...
class SqlFormatter {
    ...
    */
    escape(value: any, unquoted?: boolean | undefined): string {
        if (value == null)
    ...
    formatSelect(query: any) {
        Args.notNull(query, 'Query expression');
        Args.check(query.$select != null, 'Format select requires a valid select expression');
        // get select fields
        const selectFields = Object.keys(query.$select);
        // get select collection
        Args.check(query.$collection != null, 'Format select requires a valid collection');
        ...
        if (query.$expand && query.$expand.length) {
            query.$expand.forEach((expand: { $lookup: any; $direction: any; }) => {
    ...
    formatLookup(lookup: any, direction: any) {
        let result = '';
        ...
        if (lookup && lookup.localField && lookup.foreignField) {
            ...
            Args.check(lookup.pipeline.$match, 'Lookup match expression is null or undefined.');
            // format selection
    ...
    formatFixedSelect(query: any): string {
        ...
        const selectFields = Object.keys(query.$select);
        // validate select expression
        Args.check(selectFields.length > 0, 'Invalid query expression. A fixed query expects at least one valid select expression.');
    ...
    formatField(expr: { [x: string]: any; }) {
        Args.notNull(expr, 'Field expression');
        let name = getOwnPropertyName(expr);
        let result;
        Args.check(name != null, 'Field name cannot be empty.');
        ...
        if (typeof field === 'object') {
            ...
            if (isMethodOrNameReference(funcName)) {
                const formatFunc = (<PropertyIndexer>this)[funcName];
    ...
    formatWhere(expr: any): any {
        ...
        if (isMethodOrNameReference(name)) {
            // get format method
            const formatFunc = (<PropertyIndexer>this)[name];
            if (typeof formatFunc === 'function') {
    ...
    formatOrder(expr: { [x: string]: number; } | null) {
        if (expr == null) {
            return '';
        }
    ...
    formatInsert(query: any) {
        ...
        Args.check(insertFields.length > 0, 'Insert fields cannot be an empty array');
    ...
    formatUpdate(query: any) {
        ...
        Args.check(updateFields.length > 0, 'Update fields cannot be an empty array');
        let hasWhere = false;
    ...
    $and() {
        ...
        Args.check(conditions.length > 0, 'Expected at least one expression.');
        let result = '(';
     ...
     */
    $eq(left: string, right: string | null) {
        if (right == null) {
    ...
    $nin(left: any, right: any) {
        return `NOT ${this.$in(left, right)}`;
    }

    $ne(left: any, right: null) {
        if (right == null) {
     ...
     * @returns {string}
     */
    $round(p0: any, p1: any | undefined): string {
        if (p1 == null) {
     ...
     * @returns {string}
     */
    $regex(left: any, expr: string | any): string {
     ...
     * @returns {string}
     */
    $text(p0: string | any, p1: string | any): string {
     ...
     * @returns {string}
     */
    $indexOfBytes(p0: string, p1: string): string {
     ...
     * @returns {string}
     */
    $substr(p0: string, pos: number, length: number | undefined): string {
        if (length) {
